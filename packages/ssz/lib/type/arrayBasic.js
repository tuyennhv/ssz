...
function setChunksNode(rootNode, chunksNode, newLength, hashComps) {
    const lengthNode = newLength !== null
        ...
        : // else re-use existing node
            rootNode.right;
    const branchNode = new persistent_merkle_tree_1.BranchNode(chunksNode, lengthNode);
    if (hashComps !== null) {
        persistent_merkle_tree_1.arrayAtIndex(hashComps.byLevel, hashComps.offset).push({ src0: chunksNode, src1: lengthNode, dest: branchNode });
    }
    return branchNode;
}
